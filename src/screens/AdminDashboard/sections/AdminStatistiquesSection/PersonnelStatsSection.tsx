...
const PersonnelStatsSection = ({
  ...
  // Extraction directe des données depuis personnelStats
  const rawPersonnelData = personnelStats?.data?.detailsPersonnel || [];
  ...
  const getRoleBadge = (role: string) => {
    ...
    const colorClass =
      roleColors[role as keyof typeof roleColors] || "bg-gray-10 text-gray-700";
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
                      activeTab === tab.key
                        ...
                        : "text-gray-600 hover:text-gray-900 hover:bg-gray-5"
                    }`}
                  >
                    <tab.icon size={16} />
                    {tab.label}
